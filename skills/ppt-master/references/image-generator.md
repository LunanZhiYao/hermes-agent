...
> \* **Exception**: the `custom` backend (local API) does NOT require an API key — it only needs `CUSTOM_IMAGE_API_URL` (defaults to `http://10.0.20.130:8091/v1/images/generations`). If `IMAGE_BACKEND=custom`, skip the API key check entirely.
